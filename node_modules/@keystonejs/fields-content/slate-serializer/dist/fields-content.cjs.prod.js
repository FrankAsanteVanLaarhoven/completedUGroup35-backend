...
var slateWalker = require('../../dist/slate-walker-b99c86b5.cjs.prod.js');
  ...
  const serializedDocument = slateWalker.walkSlateNode(value.document, {
    visitBlock(node) {
      ...
      if (mutations && Object.keys(mutations).length) {
        ...
        if (!block.path) {
          throw new Error(`No mutation path set for block view type '${block.type}'. Ensure the block's view exports a 'path' key corresponding to the mutation path for saving block data`);
...
function deserialize(_ref, blocks) {
  let {
    document
  } = _ref,
      serializations = _objectWithoutProperties(_ref, ["document"]);

  assert__default['default'](!!document, 'Must pass document to deserialize()');
  assert__default['default'](!!blocks, 'Must pass blocks to deserialize()');
  ...
  return value.set('document', slateWalker.walkSlateNode(value.document, {
    visitBlock(node) {
      ...
      const joins = (nodeData && nodeData.size && nodeData.get('_joinIds') || []).map(joinId => data.find(({
        id
      }) => joinId === id)); // NOTE: deserialize _may_ return null. It will then fall into the
